      ...
      SELECT p.*,
        json_agg(
          json_build_object(
            ...
            's3_url', pm.s3_url,
            'thumbnail_s3_url', pm.thumbnail_s3_url,
...
router.post('/', async (req: AuthRequest, res) => {
  try {
    ...
    const { course_id, message_text, is_pinned } = req.body;
    ...
    const result = await query(`
      INSERT INTO course_posts (course_id, message_text, is_pinned)
      VALUES ($1, $2, $3)
      RETURNING *
    `, [course_id, message_text || '', is_pinned || false]);
...
router.put('/:id', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const userId = req.userId;
    const { message_text, is_pinned } = req.body;
    ...
    for (const item of normalizedItems) {
      const result = await query(`
        INSERT INTO course_post_media (
          post_id, media_type, s3_url, thumbnail_s3_url,
          file_size, duration_seconds, file_name, mime_type,
          media_group_id, telegram_file_id
        )
      ...
      `, [
        ...
        item.s3_url || item.storage_path || null,
...
router.patch('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    const allowedFields = ['message_text', 'is_pinned', 'has_error', 'error_message', 'media_count', 'storage_path', 'telegram_file_id', 'telegram_thumbnail', 'file_name', 'file_size', 'media_type', 'title', 'text_content'];
...
router.patch('/media/:mediaId', async (req: AuthRequest, res) => {
  try {
    ...
    const allowedFields = ['order_index', 'media_type', 's3_url', 'thumbnail_s3_url', 'file_name', 'file_size', 'mime_type'];
    for (const [key, value] of Object.entries(updates)) {
      if (allowedFields.includes(key)) {
        fields.push(`${key} = $${paramCount}`);
        values.push(value);
...
router.get('/pinned/:courseId/:studentId', async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(`
      SELECT spp.*, p.message_text, p.created_at as post_created_at
