...
import { uploadToS3, getPresignedUrl, deleteFromS3, generateS3Key, getMediaPublicUrl, getS3Object, streamS3Object } from '../utils/s3.js';
...
router.get('/telegram/:fileId/:courseId', async (req: AuthRequest, res) => {
  try {
    ...
    if (mediaResult.rows.length > 0 && mediaResult.rows[0].storage_path) {
      ...
      try {
        await serveS3Media(req, res, s3Key);
        return;
      } catch {
    ...
    if (postCheck.rows.length > 0 && postCheck.rows[0].storage_path) {
      try {
        await serveS3Media(req, res, postCheck.rows[0].storage_path);
        return;
      } catch {
...
async function serveS3Media(req: express.Request, res: express.Response, s3Key: string) {
  ...
  if (isVideo && rangeHeader) {
    const streamResult = await streamS3Object(s3Key, rangeHeader);
    if (!streamResult) {
      return res.status(404).json({ error: 'File not found' });
    }

    res.status(streamResult.statusCode === 206 ? 206 : 206);
    res.setHeader('Content-Type', contentType);
...
router.get('/s3/*', async (req, res) => {
  try {
    ...
    if (!s3Key) return res.status(400).json({ error: 'Invalid request' });
    await serveS3Media(req, res, s3Key);
  } catch (error) {
